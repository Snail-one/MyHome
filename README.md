...
需要排查图标获取时，可设置 `ICON_FETCH_LOG=true`。服务端会向标准输出打印 `[icon-fetch] https://example.com/ | direct | request:start phase=html` 这类抓取日志；第一段是目标网址，第二段是直连/代理，第三段是处理事件和参数。HTML 页面获取会显示 `html:fetch:success/fail`，图标链接解析会显示 `html:parse:success/fail`。在支持颜色的终端中，URL、请求、成功、失败和回退状态会用不同颜色区分。
