...
需要排查图标获取时，可设置 `ICON_FETCH_LOG=true`。服务端会向标准输出打印 `[icon-fetch] https://example.com/ | request:start | phase=html mode=direct` 这类抓取日志，包含直连/代理、请求阶段、HTTP 状态、候选数量和最终命中/失败结果；在支持颜色的终端中，URL、请求、成功、失败和回退状态会用不同颜色区分。
